...
from posts.templatetags.stadistics import Estimator
from posts.templatetags.preparacion import Preparation
...
def main(url):
    ...
    ans = ["Total of cells of sample: {}.".format(quantity)] #0
    #ans = ans + "\n"
    #print("Total of cells of sample: {}.".format(quantity))
    ans.append("Total of cells of population approximately: {}.".format(res)) #1
    #ans = ans + "\n"
    #print("Total of cells of population approximately: {}.".format(res))
    prep= Preparation('test1')
    pro,meta,ana,telo,uncl=prep.main()
    ans.append("Estimations with a confidence level of {}%.".format(0.9*100)) #2
    #ans = ans  + "\n"
    #print("\nEstimations with a confidence level of {}%.".format(0.9*100))
    ans.append("Estimation for cells in prophase: {}.".format(est.normal_est(pro))) #3
    #ans = ans + "\n"
    #print("Estimation for cells in prophase: {}".format(est.normal_est(pro)))
    ans.append("Estimation for cells in metaphase: {}.".format(est.normal_est(meta))) #4
    #ans = ans + "\n"
    #print("Estimation for cells in metaphase: {}".format(est.normal_est(meta)))
    ans.append("Estimation for cells in anaphase: {}.".format(est.normal_est(ana))) #5
    #ans = ans + "\n"
    #print("Estimation for cells in anaphase: {}".format(est.normal_est(ana)))
    ans.append("Estimation for cells in telophase: {}.".format(est.normal_est(telo))) #6
    #ans = ans + "\n"
